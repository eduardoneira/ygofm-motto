...
impl TrackerGridLayout {
    pub(super) fn new(columns: usize, max_visible_rows: usize) -> Self {
        Self {
    ...
    pub(super) fn initial_window_size(&self, tracker_count: usize) -> egui::Vec2 {
        self.natural_content_size(tracker_count) + WINDOW_PADDING
    }
    ...
    pub(super) fn metrics_for_available_size(
        ...
        let scale = self.scale_for_available_size(available_size, tracker_count);
        let visible_rows = self.visible_rows_for(tracker_count);
    ...
    fn scale_for_available_size(&self, available_size: egui::Vec2, tracker_count: usize) -> f32 {
        let natural_content_size = self.natural_content_size(tracker_count);
    ...
    fn natural_content_size(&self, tracker_count: usize) -> egui::Vec2 {
        grid_size(
            NATURAL_TRACKER_CONTROL_SIZE,
            NATURAL_GRID_SPACING,
            self.columns,
            self.visible_rows_for(tracker_count),
        )
    }

    fn visible_rows_for(&self, tracker_count: usize) -> usize {
        occupied_rows(tracker_count, self.columns).min(self.max_visible_rows)
    }
}

fn occupied_rows(tracker_count: usize, columns: usize) -> usize {
    tracker_count.max(1).div_ceil(columns.max(1))
}
...
mod tests {
    ...
    fn keeps_natural_scale_when_the_grid_fits() {
        ...
        let metrics = layout.metrics_for_available_size(egui::vec2(1000.0, 1000.0), 15);
    ...
    fn scales_down_to_fit_requested_rows_and_columns() {
        let layout = TrackerGridLayout::new(5, 3);
        let metrics = layout.metrics_for_available_size(layout.initial_window_size(15) / 2.0, 15);
    ...
    fn compact_window_size_shrinks_with_the_scale() {
        ...
        let metrics = layout.metrics_for_available_size(layout.initial_window_size(30) / 2.0, 30);

        assert!(metrics.compact_window_size.x < layout.initial_window_size(30).x);
        assert!(metrics.compact_window_size.y < layout.initial_window_size(30).y);
    }

    #[test]
    fn visible_rows_are_capped_by_actual_tracker_rows() {
        let layout = TrackerGridLayout::new(5, 3);
        let metrics = layout.metrics_for_available_size(egui::vec2(1000.0, 1000.0), 6);
        let expected_size =
            grid_size(NATURAL_TRACKER_CONTROL_SIZE, NATURAL_GRID_SPACING, 5, 2) + WINDOW_PADDING;

        assert_eq!(metrics.compact_window_size, expected_size);
    }

    #[test]
    fn never_scales_below_the_readable_minimum() {
        let layout = TrackerGridLayout::new(20, 20);
        let metrics = layout.metrics_for_available_size(egui::vec2(100.0, 100.0), 400);
